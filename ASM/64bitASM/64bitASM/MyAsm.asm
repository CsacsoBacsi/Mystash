...
positive1:	xor		rax, rax
	...
	equal:	xor		rax, rax				; Could not find
			...
			; Exponent (bit 30 – 23 – 8 bits): 10000100 = 132 which is with the bias of 127 gives us 132 – bias = 5 so 2^5
			; Mantissa (bit : 22 – 0 – 23 bits) = 01011101000000000000000. The initial 1. is implied so no need to store it
			; So:
			; 1.01011101 * 2^5
			; = 101011.101 i.e.: 2^5 + 2^3 + 2^1 + 2^0 . + 2^-1 + 2^-3
			...
			; What is left on the right we take. The fractional part: 0.390625
			...
			; The binary representation of the number then: 0.011001. Together with the whole number on the left of the original number (329) it looks as this: 101001001.011001
