...
#
# "Double" decorator which decorates before and also after the decorated function call
#
def f_decorator_d (func): # Parameter is a function (to be decorated), return value is a function too which wraps the func parameter function
    def f_wrapper (p_param): # The number of parameters must match that of the params of the dunction to be decorated because basically the f_wrapper will be called
        print ("*** This is the before decoration ***")
        p_param -= 1
        retval = func (p_param) # func does its job by default. The previous two lines are the decoration
        print ("*** This is the after decoration ***")
        return retval
    return f_wrapper # Returns the wrapper function

def func_to_be_decorated_d (p_param):
    p_param *= p_param
    return p_param

wrapper_func_d = f_decorator_d(func_to_be_decorated_d)
print (wrapper_func_d (7)) # Result must be (7 - 1) ^ 2 = 36
