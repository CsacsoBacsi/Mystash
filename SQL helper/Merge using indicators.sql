...
insert into csaba_trg (col1, col2, col3, val, start_date, end_date)
values ('1','1','1',1, to_date ('01/01/2019', 'DD/MM/YYYY'), to_date ('31/03/2025', 'DD/MM/YYYY')) ;
insert into csaba_trg (col1, col2, col3, val, start_date, end_date)
values ('1','1','1',1, to_date ('01/04/2025', 'DD/MM/YYYY'), NULL) ;
insert into csaba_trg (col1, col2, col3, val, start_date, end_date)
values ('2','1','1',1, to_date ('01/01/2019', 'DD/MM/YYYY'), NULL) ;
insert into csaba_trg (col1, col2, col3, val, start_date, end_date)
values ('3','1','1',1, to_date ('01/01/2019', 'DD/MM/YYYY'), NULL) ;
...
INTO csaba_trg AS t USING  
    ...
    SELECT ua.col1, ua.col2, ua.col3, ua.val, 
      ...
      FROM  ua 
      GROUP BY ua.col1, ua.col2, ua.col3, ua.val 
      HAVING COUNT (ua.ind1) != COUNT (ua.ind2)) AS src 
ON (t.col1 || '||' || t.col2 || '||' || t.col3 || '||' || 'U' = 
    src.col1 || '||' || src.col2 || '||' || src.col3 || '||' || src.transaction_type) 
WHEN MATCHED AND t.end_date IS NULL THEN UPDATE  
    SET end_date = src.end_date
WHEN NOT MATCHED AND transaction_type = 'I' THEN INSERT (col1, col2, col3, val, 
                              start_date, end_date) 
                      VALUES (src.col1, src.col2, src.col3, src.val, 
                              src.start_date,
                              NULL) 
